...
class Trader:
    ...
    def get_wall_prices(self, order_depth: OrderDepth):
        bid_wall_price = None
        ask_wall_price = None
        ...
        if order_depth.buy_orders:
            ...
            if total_bid_vol > 0:
                bid_wall_price = sum(price * vol for price, vol in order_depth.buy_orders.items()) / total_bid_vol

        if order_depth.sell_orders:
            total_ask_vol = sum(-vol for vol in order_depth.sell_orders.values())
            if total_ask_vol > 0:
                ask_wall_price = sum(price * (-vol) for price, vol in order_depth.sell_orders.items()) / total_ask_vol
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            if product == "INTARIAN_PEPPER_ROOT":
                ...
                avg_ask_price = (
                    sum(traderObject["intarian_pepper_root_last_ask_price_history"])
                    / len(traderObject["intarian_pepper_root_last_ask_price_history"])
                    if len(traderObject["intarian_pepper_root_last_ask_price_history"]) > 0
                    else 0
                )
                fair_ask_value = (
                    avg_ask_price + 0.1 * (len(traderObject["intarian_pepper_root_last_ask_price_history"]) + 1) / 2 + 0.1
                    if avg_ask_price != 0
                    else 0
                )
                avg_bid_price = (
                    sum(traderObject["intarian_pepper_root_last_bid_price_history"])
                    / len(traderObject["intarian_pepper_root_last_bid_price_history"])
                    if len(traderObject["intarian_pepper_root_last_bid_price_history"]) > 0
                    else 0
                )
                fair_bid_value = (
                    avg_bid_price + 0.1 * (len(traderObject["intarian_pepper_root_last_bid_price_history"]) + 1) / 2 + 0.1
                    if avg_bid_price != 0
                    else 0
                )
                fair_price = (
                    fair_ask_value * fair_value_coeff + fair_bid_value * (1 - fair_value_coeff)
                    if (fair_ask_value != 0 and fair_bid_value != 0)
                    else 0
                )
                ...
                # =====================================================
                # ASH: rolling-mid fair + wall-based stale quote capture
                # =====================================================
                if "ash_mid_history" not in traderObject:
                    traderObject["ash_mid_history"] = []
                if "ash_bid_wall_history" not in traderObject:
                    traderObject["ash_bid_wall_history"] = []
                if "ash_ask_wall_history" not in traderObject:
                    traderObject["ash_ask_wall_history"] = []
                ...
                # ---------- fair from rolling mid ----------
                mid_hist = traderObject["ash_mid_history"]
                mid_hist.append(mid_price)
                if len(mid_hist) > 20:
                    mid_hist.pop(0)

                fair_value = sum(mid_hist) / len(mid_hist)

                if len(mid_hist) >= 5:
                    mean_hist = fair_value
                    variance = sum((x - mean_hist) ** 2 for x in mid_hist) / len(mid_hist)
                ...
                # ---------- wall prices for stale detection only ----------
                bid_wall_price, ask_wall_price = self.get_wall_prices(order_depth)
                if bid_wall_price is None:
                    bid_wall_price = best_bid
                if ask_wall_price is None:
                    ask_wall_price = best_ask

                bid_wall_hist = traderObject["ash_bid_wall_history"]
                ask_wall_hist = traderObject["ash_ask_wall_history"]

                bid_wall_hist.append(bid_wall_price)
                ask_wall_hist.append(ask_wall_price)

                if len(bid_wall_hist) > 20:
                    bid_wall_hist.pop(0)
                if len(ask_wall_hist) > 20:
                    ask_wall_hist.pop(0)

                avg_bid_wall = sum(bid_wall_hist) / len(bid_wall_hist)
                avg_ask_wall = sum(ask_wall_hist) / len(ask_wall_hist)

                # relative to wall, not mid
                wall_ask_gap = avg_ask_wall - best_ask   # positive if best ask is unusually cheap
                wall_bid_gap = best_bid - avg_bid_wall   # positive if best bid is unusually rich
                ...
                # Track whether aggressive order already fired this tick
                aggressive_buy_done = False
                aggressive_sell_done = False

                # =====================================================
                # 1) wall-based stale quote capture
                # =====================================================
                # If best ask is much cheaper than ask wall, hit it.
                # If best bid is much richer than bid wall, hit it.
                wall_stale_threshold = 2.0
                stale_take_size = 12

                if wall_ask_gap >= wall_stale_threshold and buy_remaining > 0:
                    qty = min(best_ask_vol, stale_take_size)
                    got = place_buy(best_ask, qty, "WALL_STALE_ASK")
                    if got > 0:
                        aggressive_buy_done = True

                if wall_bid_gap >= wall_stale_threshold and sell_remaining > 0:
                    qty = min(best_bid_vol, stale_take_size)
                    got = place_sell(best_bid, qty, "WALL_STALE_BID")
                    if got > 0:
                        aggressive_sell_done = True

                # =====================================================
                # 2) target position from z-score mean reversion
                # =====================================================
                ...
                # avoid stacking aggressive same-side orders in the same tick
                if not aggressive_buy_done and inventory_gap > 0 and z_ask <= -mr_entry_threshold and buy_remaining > 0:
                    qty = min(best_ask_vol, taker_size, inventory_gap)
                    got = place_buy(best_ask, qty, "MR_TAKER_BUY")
                    if got > 0:
                        aggressive_buy_done = True

                if not aggressive_sell_done and inventory_gap < 0 and z_bid >= mr_entry_threshold and sell_remaining > 0:
                    qty = min(best_bid_vol, taker_size, -inventory_gap)
                    got = place_sell(best_bid, qty, "MR_TAKER_SELL")
                    if got > 0:
                        aggressive_sell_done = True
                ...
                # If already did aggressive same-side trade this tick, suppress passive same-side quote
                ...
                logger.print(
                    f"[ASH WALL+MR] ts={state.timestamp} "
                    ...
                    f"bid_wall={bid_wall_price:.2f} ask_wall={ask_wall_price:.2f} "
                    f"avg_bid_wall={avg_bid_wall:.2f} avg_ask_wall={avg_ask_wall:.2f} "
                    f"wall_bid_gap={wall_bid_gap:.2f} wall_ask_gap={wall_ask_gap:.2f} "
                    f"bb={best_bid} ba={best_ask} bid_q={bid_quote} ask_q={ask_quote} "
                    f"buy_sz={buy_size} sell_sz={sell_size} "
                    f"aggr_buy={aggressive_buy_done} aggr_sell={aggressive_sell_done}"
                )
