...
def make_horizon_pairs(df: pd.DataFrame, horizon: int) -> pd.DataFrame:
    ...
    for _, g in df.groupby("day", sort=False):
        ...
        pairs.append(sub[["r_past", "r_future"]])
...
def main():
    ...
    pairs = make_horizon_pairs(df, HORIZON)
    ...
    x_plot = plot_df["r_past"].to_numpy()
    y_plot = plot_df["r_future"].to_numpy()
    ...
    plt.title(
        f"{TARGET_SYMBOL}: {HORIZON}-timeclock mean reversion test\n"
