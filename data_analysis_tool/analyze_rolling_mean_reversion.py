...
ROLLING_WINDOWS = [10, 20, 30, 50, 75, 100, 125, 150, 175, 200, 250, 300]

# True: rolling_mean_t에 현재 mid_t 포함
# False: rolling_mean_t는 t 이전 값들만 사용
USE_CURRENT_MEAN = True
...
def rolling_mean_series(values: pd.Series, window: int, use_current: bool) -> pd.Series:
    if use_current:
        return values.rolling(window=window, min_periods=window).mean()

    return values.shift(1).rolling(window=window, min_periods=window).mean()


def make_rolling_mean_pairs(df: pd.DataFrame, rolling_window: int) -> pd.DataFrame:
    frames = []
    ...
    for day, g in df.groupby("day", sort=True):
        ...
        g["rolling_mean"] = rolling_mean_series(
            g["analysis_mid"],
            rolling_window,
            USE_CURRENT_MEAN,
        )
        g["residual"] = g["analysis_mid"] - g["rolling_mean"]

        g["future_mid"] = g["analysis_mid"].shift(-HORIZON)
        g["future_gap"] = g["timestamp"].shift(-HORIZON) - g["timestamp"]
        g["future_change"] = g["future_mid"] - g["analysis_mid"]

        valid = (
            (g["future_gap"] == HORIZON * step)
...
def regression_stats(x: np.ndarray, y: np.ndarray) -> dict:
    ...
    return {
        ...
        "mean_abs_residual": float(np.mean(np.abs(x))),
...
def plot_scatter_with_fit(
    ...
    x = plot_df["residual"].to_numpy(dtype=float)
    ...
    plt.xlabel(f"residual = mid_t - rolling_mean_t, window={rolling_window}")
    plt.ylabel("future_change = mid_(t+1) - mid_t")
    plt.title(
        f"{PRODUCT} | {day_label} | rolling={rolling_window} | 1-tick MR\n"
        f"beta={beta:.4f}, corr={stats['corr']:.4f}, "
        f"r2={stats['r2']:.4f}, hit={stats['hit_rate']:.3f}"
    )
    plt.tight_layout()

    out_path = out_dir / f"{day_label}_rolling{rolling_window}_1_tick_rolling_mean.png"
    plt.savefig(out_path, dpi=160)
    plt.close()


def analyze_one_day(df: pd.DataFrame, day_label: str, out_dir: Path) -> pd.DataFrame:
    rows = []

    for rolling_window in ROLLING_WINDOWS:
        pairs = make_rolling_mean_pairs(df, rolling_window)
        ...
        if pairs.empty:
            ...
            print(f"{day_label:>7} | window={rolling_window:>3} | no valid pairs")
        else:
            stats = regression_stats(
                x=pairs["residual"].to_numpy(dtype=float),
            ...
            print(
                f"{day_label:>7} | window={rolling_window:>3} | "
                f"n={stats['n']}, beta={stats['beta']:.4f}, corr={stats['corr']:.4f}, "
                f"r2={stats['r2']:.4f}, t={stats['t_beta']:.1f}, hit={stats['hit_rate']:.3f}"
            )
            print(
                f"{'':>7} | {'':>10} | "
                f"mean_abs_residual={stats['mean_abs_residual']:.4f}, "
...
def main() -> None:
    ...
    print(f"\n=== {PRODUCT} rolling mean 1-tick reversion analysis ===")
    print(f"ROUND={ROUND}, DATA_DIR={DATA_DIR}")
    print(f"USE_CURRENT_MEAN={USE_CURRENT_MEAN}")
    print(f"saved to: {output_root}\n")

    all_summaries.append(analyze_one_day(df, "day_all", output_root))

    for day in sorted(df["day"].unique()):
        day_df = df[df["day"] == day].copy()
        day_label = f"day_{int(day)}"
        all_summaries.append(analyze_one_day(day_df, day_label, output_root))

    summary_df = pd.concat(all_summaries, ignore_index=True)
    summary_path = output_root / "rolling_mean_reversion_1_tick_summary.csv"
