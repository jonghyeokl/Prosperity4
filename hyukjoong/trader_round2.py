...
class Trader:
    ...
    # ===================================================================
    # 1) bid() 함수 추가 — Market Access Fee 입찰
    # 2) VALID_BID_ASK_VOLUME 6 으로 완화 (80% 호가 환경 + 추가 25% 호가)
    # 3) 나머지 파라미터는 전부 라운드 1 튜닝값 유지
    # ===================================================================

    # !!! 입찰가: 라운드 1 PnL에 맞춰 수정하세요 !!!
    # 권장: 라운드 1 PnL의 5~15% 사이
    # 예시: R1 PnL = 50,000 이면 MAF_BID = 5000 ~ 7500
    #       R1 PnL = 100,000 이면 MAF_BID = 7000 ~ 15000
    # 너무 낮으면(<1000) 상위 50% 놓침, 너무 높으면(>20000) 수익 낭비
    MAF_BID = 5000
    ...
    # (변경) 10 → 6: 80% 호가 환경에서 high-volume 호가가 드묾 + 추가 25% 호가도 포착
    VALID_BID_ASK_VOLUME = 6
    ...
    # =================================================================
    # Market Access Fee 입찰 함수 (Round 2 전용)
    # =================================================================
    def bid(self):
        return self.MAF_BID
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            if product == "INTARIAN_PEPPER_ROOT":
                ...
                if len(past_few_mid_history) >= self.ASH_MIN_HISTORY_LENGTH:
                    # fair_price: 중앙값
                ...
                if valid_mid_price is not None:
                    if len(past_few_mid_history) < self.ASH_MAX_HISTORY_LENGTH:
