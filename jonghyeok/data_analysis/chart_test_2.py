...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "trade volume filtering, and optional spread event vertical lines."
        )
    )
    ...
    parser.add_argument(
        "--spread-threshold",
        type=float,
        default=SPREAD_LINE_THRESHOLD,
        help=f"Draw vertical lines where ask_price_1 - bid_price_1 <= threshold. Default: {SPREAD_LINE_THRESHOLD}",
    )
...
def build_spread_event_shapes(product_day_prices: pd.DataFrame, spread_threshold: float) -> list[dict[str, object]]:
    spread = product_day_prices["ask_price_1"] - product_day_prices["bid_price_1"]
    event_timestamps = product_day_prices.loc[spread <= spread_threshold, "timestamp"].tolist()
...
def build_product_day_figure(
    ...
    spread_shapes = build_spread_event_shapes(product_day_prices, spread_threshold)
    ...
    return fig, trade_payload, len(spread_shapes)
...
def build_html_document(figures: list[dict[str, object]], title: str, spread_threshold: float) -> str:
    chunks = [
        "<!DOCTYPE html>",
        "<html lang='en'>",
        "<head>",
        ...
        "    .sidebar .checkbox-row { display: flex; align-items: center; gap: 8px; margin-top: 16px; }",
        "    .sidebar .checkbox-row input { width: auto; padding: 0; margin: 0; }",
        "    .sidebar .checkbox-row label { margin: 0; font-size: 14px; }",
        ...
        "  <p>Legend click: toggle traces. Drag: zoom. Double click: reset. The trade filter and spread-line toggle apply to all charts.</p>",
        "  <div class='sidebar'>",
        ...
        "      <input id='showSpreadLines' type='checkbox' checked>",
        "      <label for='showSpreadLines'>Show spread ≤ " + escape(str(spread_threshold)) + " lines</label>",
        "    </div>",
        "    <div class='hint'>Light purple vertical line = ask_price_1 - bid_price_1 ≤ threshold at that timestamp.</div>",
        "  </div>",
    ]
        ...
        figure_meta.append(
            {
                ...
                "product": product,
                "day": day,
                "spread_shape_count": item["spread_shape_count"],
            }
        )
    ...
    function setSpreadLinesVisible(isVisible) {
      FIGURE_META.forEach((meta) => {
        const gd = document.getElementById(meta.div_id);
        if (!gd || !meta.spread_shape_count) return;
        const update = {};
        for (let i = 0; i < meta.spread_shape_count; i++) {
          update[`shapes[${i}].visible`] = isVisible;
        }
    ...
    document.getElementById('showSpreadLines').addEventListener('change', (event) => {
      setSpreadLinesVisible(event.target.checked);
    });
...
def main() -> None:
    ...
    for product in products:
        ...
        for day in days:
            ...
            fig, trade_payload, spread_shape_count = build_product_day_figure(
                product,
                day,
                product_day_prices,
                product_day_trades,
            ...
            figures.append(
                {
                    ...
                    "spread_shape_count": spread_shape_count,
                }
            )
