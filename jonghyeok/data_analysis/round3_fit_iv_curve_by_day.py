...
def fit_by_day(filtered: pd.DataFrame, weighted: bool) -> tuple[pd.DataFrame, dict]:
    fitted_frames = []
    day_results = {}

    for file_idx, g in filtered.groupby("file_idx"):
        if len(g) < 20:
            print(f"[SKIP] file_idx={file_idx}: too few points {len(g)}")
            continue
    ...
    if not fitted_frames:
        raise RuntimeError("No day fits produced.")

    return pd.concat(fitted_frames, ignore_index=True), day_results
...
def save_outputs(
    ...
    payload = {
        "fit_type": "weighted_quadratic_by_day" if weighted else "quadratic_by_day",
        "formula": "fair_iv(m) = a*m^2 + b*m + c",
    ...
    coeff_path = out_dir / "round3_iv_fit_coeffs_by_day.json"
    points_path = out_dir / "round3_iv_points_with_fit_by_day.csv"

    coeff_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    fitted.to_csv(points_path, index=False)

    # 1. day별 scatter + 각 day fitting curve
    for day_key, info in day_results.items():
        day_df = fitted[fitted["file_idx"] == info["file_idx"]].copy()
        coeffs = np.array(info["coeffs"], dtype=float)

        xs = np.linspace(day_df["m"].min(), day_df["m"].max(), 500)
        ys = np.polyval(coeffs, xs)

        plt.figure(figsize=(11, 7))

        for product, g in day_df.groupby("product"):
            plt.scatter(g["m"], g["raw_iv"], s=8, alpha=0.35, label=product)

        plt.plot(xs, ys, linewidth=2.5, label=f"{day_key} quadratic fit")
        plt.xlabel("m = log(K / S) / sqrt(TTE)")
        plt.ylabel("raw implied volatility")
        plt.title(f"Round 3 IV Smile Fit - {day_key}")
        plt.legend(ncol=2, fontsize=8)
        plt.tight_layout()
        plt.savefig(out_dir / f"round3_iv_fit_scatter_{day_key}.png", dpi=160)
        plt.close()

        # residual plot
        plt.figure(figsize=(11, 6))
        plt.scatter(day_df["m"], day_df["residual"], s=8, alpha=0.35)
        plt.axhline(0.0, linewidth=1)
        plt.xlabel("m")
        plt.ylabel("raw_iv - fit_iv")
        plt.title(f"IV Fit Residuals - {day_key}")
        plt.tight_layout()
        plt.savefig(out_dir / f"round3_iv_fit_residuals_{day_key}.png", dpi=160)
        plt.close()

    # 2. 모든 day의 quadratic curve를 하나의 그래프에 비교
    global_min_m = float(fitted["m"].min())
    global_max_m = float(fitted["m"].max())
    xs = np.linspace(global_min_m, global_max_m, 800)

    plt.figure(figsize=(11, 7))

    for day_key, info in sorted(day_results.items(), key=lambda x: x[1]["file_idx"]):
        coeffs = np.array(info["coeffs"], dtype=float)
        ys = np.polyval(coeffs, xs)
    ...
    plt.title("Round 3 Quadratic IV Curves by Day")
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(out_dir / "round3_iv_fit_curves_by_day.png", dpi=180)
    plt.close()

    # 3. day별 residual boxplot
    plt.figure(figsize=(10, 6))
    labels = []
    data = []

    for day_key, info in sorted(day_results.items(), key=lambda x: x[1]["file_idx"]):
        day_df = fitted[fitted["file_idx"] == info["file_idx"]]
        labels.append(day_key)
        data.append(day_df["residual"].to_numpy())

    plt.boxplot(data, labels=labels, showfliers=False)
    plt.axhline(0.0, linewidth=1)
    plt.ylabel("raw_iv - fit_iv")
    plt.title("Residual Distribution by Day")
    plt.tight_layout()
    plt.savefig(out_dir / "round3_iv_fit_residuals_by_day.png", dpi=160)
    plt.close()

    print(f"\n[SAVED] {coeff_path}")
    print(f"[SAVED] {points_path}")
    print(f"[SAVED] {out_dir / 'round3_iv_fit_curves_by_day.png'}")


def main() -> None:
    repo_root = Path(__file__).resolve().parents[2]
    default_data_dir = repo_root / "data_capsule" / "round3"
    default_out_dir = Path(__file__).resolve().parent / "outputs" / "round3_iv_fit_by_day"
    ...
    fitted, day_results = fit_by_day(filtered, weighted=args.weighted)
    save_outputs(fitted, day_results, args.out_dir, weighted=args.weighted)
