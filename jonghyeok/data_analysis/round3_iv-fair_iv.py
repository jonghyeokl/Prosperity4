# jonghyeok/data_analysis/round3_plot_iv_residual_html.py
...
def collect_iv_residual_points(data_dir: Path) -> pd.DataFrame:
    ...
    for path in price_files:
        ...
        for ts, r in pivot.iterrows():
            ...
            for product, K in VOUCHERS.items():
                ...
                raw_iv = implied_vol_bisect(S=S, K=K, T=T, price=V)

                if raw_iv is None or not np.isfinite(raw_iv):
                    continue

                fit_iv = fair_iv(S=S, K=K, T=T)
                iv_residual = raw_iv - fit_iv
    ...
    if df.empty:
        raise RuntimeError("No IV residual points collected.")
    ...
    for product in VOUCHERS:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=g["iv_residual"],
                mode="markers",
                name=f"{product} raw_iv - fair_iv",
                marker={"size": 4, "opacity": 0.55},
                customdata=g[["day", "timestamp", "S", "voucher_price", "tte_days", "raw_iv", "fair_iv"]].to_numpy(),
                hovertemplate=(
                    f"{product}<br>"
                    "combined_ts=%{x}<br>"
                    "raw_iv - fair_iv=%{y:.6f}<br>"
                    "day=%{customdata[0]}<br>"
                    "timestamp=%{customdata[1]}<br>"
                    "S=%{customdata[2]:.2f}<br>"
                    "voucher_price=%{customdata[3]:.2f}<br>"
                    "TTE_days=%{customdata[4]:.4f}<br>"
                    "raw_iv=%{customdata[5]:.6f}<br>"
                    "fair_iv=%{customdata[6]:.6f}<extra></extra>"
                ),
            )
        )
        ...
        fig.update_layout(
            title=f"{product}: raw IV - fair IV",
            xaxis_title="combined timestamp",
            yaxis_title="raw_iv - fair_iv",
            dragmode="zoom",
            hovermode="closest",
...
def main() -> None:
    ...
    df = collect_iv_residual_points(data_dir)

    csv_path = out_dir / "round3_iv_residual_points_5000_5500.csv"
    html_path = out_dir / "round3_iv_residual_5000_5500.html"

    df.to_csv(csv_path, index=False)

    html = build_html(df, title="Round 3 IV Residual: raw_iv - fair_iv")
