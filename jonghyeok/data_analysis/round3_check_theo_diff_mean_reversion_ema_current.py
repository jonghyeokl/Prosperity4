# jonghyeok/data_analysis/round3_check_theo_diff_mean_reversion_ema_current.py
...
def ema_current(values: pd.Series, window: int) -> pd.Series:
    """
    t 시점 residual 계산에 현재 theo_diff_t를 포함한 EMA_t 사용.
    실제 Trader 코드의 ema()와 같은 방식.
    """
...
def main() -> None:
    ...
    # voucher 한꺼번에 fitting한 IV curve 기준으로 만든 theo_diff 파일
    input_path = (
        project_root
        / "jonghyeok"
        / "data_analysis"
        / "output"
        / "round3_theo_diff_5000_5300+5500.csv"
    )
    ...
    out_dir = (
        ...
        / "data_analysis"
        / "output"
        / "theo_diff_mean_reversion_ema_current"
    )
    ...
    for product, base_g in df.groupby("product"):
        ...
        for ema_window in EMA_WINDOWS:
            ...
            # 핵심 변경: EMA_prev가 아니라 EMA_t 사용
            g["ema_current"] = ema_current(g["theo_diff"], ema_window)
            g["residual"] = g["theo_diff"] - g["ema_current"]
    ...
    reg_path = out_dir / "theo_diff_mean_reversion_regression_grid_ema_current.csv"
    bucket_path = out_dir / "theo_diff_mean_reversion_buckets_grid_ema_current.csv"

    reg_df.to_csv(reg_path, index=False)
    bucket_df.to_csv(bucket_path, index=False)

    print("\n=== Regression Summary: EMA_t current included ===")
    print(
        reg_df[
            [
                "product",
