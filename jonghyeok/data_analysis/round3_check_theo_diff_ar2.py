# jonghyeok/data_analysis/round3_check_theo_diff_ar2.py
...
def fit_ar2(g: pd.DataFrame) -> dict:
    ...
    # AR(2) stability: lambda^2 - a*lambda - b = 0 의 root 절댓값이 1보다 작으면 안정적
...
def fit_train_test_ar2(g: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "test_r2": float(r2),
        "test_corr_pred_actual": float(np.corrcoef(y_test, pred)[0, 1]),
    }
...
def plot_product(g: pd.DataFrame, product: str, info: dict, out_dir: Path) -> None:
    ...
    coef = np.array([info["a"], info["b"], info["c"]], dtype=float)
    pred = X @ coef

    sample_n = min(5000, len(g))
    idx = np.random.default_rng(42).choice(len(g), size=sample_n, replace=False)
    ...
    plt.title(
        f"{product}: AR(2) theo_diff prediction\n"
        f"x_t = {info['a']:.4f} x_(t-1) + {info['b']:.4f} x_(t-2) + {info['c']:.4f}, "
        f"r2={info['r2']:.4f}"
    )
    plt.tight_layout()
    plt.savefig(out_dir / f"round3_theo_diff_ar2_pred_actual_{product}.png", dpi=160)
    plt.close()
...
def main() -> None:
    ...
    for product, g in ar2.groupby("product"):
        info = fit_ar2(g)
        test_info = fit_train_test_ar2(g)
    ...
    ar2_path = out_dir / "round3_theo_diff_ar2_rows.csv"
    summary_path = out_dir / "round3_theo_diff_ar2_summary.csv"

    ar2.to_csv(ar2_path, index=False)
    summary.to_csv(summary_path, index=False)

    print("\n=== Theo Diff AR(2) Summary ===")
    print(
        summary[
            [
                "product",
