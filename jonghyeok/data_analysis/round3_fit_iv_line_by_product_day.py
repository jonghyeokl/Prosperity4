...
def fit_linear(df: pd.DataFrame, weighted: bool) -> FitResult:
...
def fit_by_product_day(filtered: pd.DataFrame, weighted: bool) -> tuple[pd.DataFrame, dict]:
    ...
    for product, pg in filtered.groupby("product"):
        ...
        for file_idx, g in pg.groupby("file_idx"):
            ...
            fit = fit_linear(g, weighted=weighted)
            fitted = add_predictions(g, fit.coeffs)
            fitted_frames.append(fitted)

            a, b = [float(x) for x in fit.coeffs]
            ...
            results[product][day_key] = {
                ...
                "file": file_name,
                "fit_type": "weighted_linear_by_product_day" if weighted else "linear_by_product_day",
                "formula": "fair_iv_product_day(m) = a*m + b",
                "product": product,
                "K": int(g["K"].iloc[0]),
                "a": a,
                "b": b,
            ...
            print(f"\n========== {product} / {day_key} ==========")
            print(f"fair_iv(m) = {a:.10f} * m + {b:.10f}")
            print(f"N    = {fit.n}")
...
def save_outputs(fitted: pd.DataFrame, results: dict, out_dir: Path, weighted: bool) -> None:
    ...
    coeff_path = out_dir / "round3_iv_fit_coeffs_linear_by_product_day.json"
    points_path = out_dir / "round3_iv_points_with_fit_linear_by_product_day.csv"

    coeff_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    fitted.to_csv(points_path, index=False)

    # 1. product별: day linear fitting curve 비교
    for product, day_infos in results.items():
        ...
        for day_key, info in sorted(day_infos.items(), key=lambda x: x[1]["file_idx"]):
            ...
            label = f"{day_key}: a={coeffs[0]:.4f}, b={coeffs[1]:.4f}"
            plt.plot(xs, ys, linewidth=2.5, label=label)

        plt.xlabel("m = log(K / S) / sqrt(TTE)")
        plt.ylabel("fitted fair IV")
        plt.title(f"Round 3 Linear IV Curves by Day - {product}")
        plt.legend(fontsize=8)
        plt.tight_layout()
        plt.savefig(out_dir / f"round3_iv_fit_linear_curves_by_day_{safe_name(product)}.png", dpi=180)
        plt.close()

    # 2. product-day별 scatter + fitted line
    for product, day_infos in results.items():
        for day_key, info in day_infos.items():
            ...
            plt.plot(xs, ys, linewidth=2.5, label=f"{product} {day_key} linear fit")
            plt.xlabel("m = log(K / S) / sqrt(TTE)")
            plt.ylabel("raw implied volatility")
            plt.title(f"Round 3 Linear IV Fit - {product} / {day_key}")
            plt.legend(fontsize=8)
            plt.tight_layout()
            plt.savefig(out_dir / f"round3_iv_fit_linear_scatter_{safe_name(product)}_{day_key}.png", dpi=160)
            plt.close()
            ...
            plt.title(f"Linear IV Fit Residuals - {product} / {day_key}")
            plt.tight_layout()
            plt.savefig(out_dir / f"round3_iv_fit_linear_residuals_{safe_name(product)}_{day_key}.png", dpi=160)
            plt.close()
    ...
    if data:
        ...
        plt.ylabel("raw_iv - fit_iv")
        plt.title("Linear Residual Distribution by Product-Day")
        plt.tight_layout()
        plt.savefig(out_dir / "round3_iv_fit_linear_residuals_by_product_day.png", dpi=160)
        plt.close()

    print(f"\n[SAVED] {coeff_path}")
    print(f"[SAVED] {points_path}")
    print(f"[SAVED] product-day linear comparison plots in {out_dir}")


def main() -> None:
    repo_root = Path(__file__).resolve().parents[2]
    default_data_dir = repo_root / "data_capsule" / "round3"
    default_out_dir = Path(__file__).resolve().parent / "outputs" / "round3_iv_fit_linear_by_product_day"
