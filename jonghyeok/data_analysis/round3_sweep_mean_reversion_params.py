...
SMILE_WINDOW_CANDIDATES = [5, 10, 15]
...
BETA_CANDIDATES = [-0.985, -0.99, -0.995, -1.0, -1.005, -1.01]
        ...
        for product in TARGET_PRODUCTS:
            for ema_window in EMA_WINDOW_CANDIDATES:
                for beta in BETA_CANDIDATES:
                    ...
                    if key in seen:
                        print(f"[SKIP {run_idx}/{total_runs}] {key}")
                        continue

                    print(
                        f"[RUN {run_idx}/{total_runs}] "
                        f"smile={smile_window}, product={product}, "
                        f"ema={ema_window}, beta={beta}"
                    )
                    ...
                    print(f"  -> {product}_3d_pnl={pnls[product]:,}")
