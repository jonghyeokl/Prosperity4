# jonghyeok/data_analysis/round4_make_day_voucher_history_map.py
...
def load_round4_prices() -> pd.DataFrame:
    project_root = Path(__file__).resolve().parents[2]

    candidate_dirs = [
        project_root / "data_capsule" / "round4",
        project_root / "data" / "round4",
        project_root / "jonghyeok" / "data" / "round4",
        project_root / "jonghyeok" / "data_analysis" / "data" / "round4",
    ]

    csv_paths = []
    for d in candidate_dirs:
        if d.exists():
            csv_paths = sorted(d.glob("prices_round_4_day_*.csv"))
            if csv_paths:
                break

    if not csv_paths:
        raise FileNotFoundError(
            "prices_round_4_day_*.csv 파일을 찾지 못했습니다. "
...
def build_history_from_source_day(df: pd.DataFrame, source_day: int) -> dict[str, list[tuple[float, float]]]:
    ...
    for product, K in IV_CURVE_FIT_VOUCHERS.items():
        ...
        for _, row in merged.iterrows():
            ...
            raw_points.append((float(m), float(iv)))

        max_raw_len = min(PRELOAD_POINTS_PER_VOUCHER, SMILE_WINDOW_PER_VOUCHER)
        raw_points = raw_points[-max_raw_len:]

        # raw point를 2개씩 묶어서 중점만 저장
        # 예: (..., 999400) + (..., 999500), (..., 999600) + (..., 999700)
        midpoint_points: list[tuple[float, float]] = []
        for i in range(0, len(raw_points) - 1, 2):
            m1, iv1 = raw_points[i]
            m2, iv2 = raw_points[i + 1]

            midpoint_m = 0.5 * (m1 + m2)
            midpoint_iv = 0.5 * (iv1 + iv2)

            midpoint_points.append(
                (round(float(midpoint_m), 10), round(float(midpoint_iv), 10))
            )

        history[product] = midpoint_points
...
def main() -> None:
    ...
    df = load_round4_prices()
    history_map = make_day_voucher_history_map(df)

    output_path = out_dir / "round4_day_voucher_history_map.py"

    with open(output_path, "w", encoding="utf-8") as f:
        f.write("# Auto-generated by round4_make_day_voucher_history_map.py\n")
