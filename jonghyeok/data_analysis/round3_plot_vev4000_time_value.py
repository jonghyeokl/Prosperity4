...
def plot_html(df: pd.DataFrame, out_path: Path) -> None:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="lines",
            name="mid - intrinsic",
            customdata=df[["day", "timestamp", "mid", "intrinsic", "raw_iv", "S", "tte_days"]].to_numpy(),
            hovertemplate=(
                "time value<br>"
                "combined_ts=%{x}<br>"
                "mid-intrinsic=%{y:.4f}<br>"
                "day=%{customdata[0]}<br>"
                "timestamp=%{customdata[1]}<br>"
                "mid=%{customdata[2]:.4f}<br>"
                "intrinsic=%{customdata[3]:.4f}<br>"
                "raw_iv=%{customdata[4]:.6f}<br>"
                "S=%{customdata[5]:.4f}<br>"
                "TTE_days=%{customdata[6]:.4f}<extra></extra>"
            ),
        )
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            customdata=low_iv[["day", "timestamp", "mid", "intrinsic", "raw_iv", "S", "tte_days"]].to_numpy(),
            hovertemplate=(
                "low IV point<br>"
                "combined_ts=%{x}<br>"
                "mid-intrinsic=%{y:.4f}<br>"
    ...
    fig.update_layout(
        title="VEV_4000 time value: mid - intrinsic",
        xaxis_title="combined timestamp",
        yaxis_title="mid - intrinsic",
        dragmode="zoom",
        hovermode="closest",
...
def main() -> None:
    ...
    csv_path = out_dir / "round3_vev4000_time_value.csv"
    html_path = out_dir / "round3_vev4000_time_value.html"
