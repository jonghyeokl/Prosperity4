...
def fit_quadratic(df: pd.DataFrame) -> FitResult:
...
def fit_by_product(filtered: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    fitted_frames = []
    product_results = {}

    for product, g in filtered.groupby("product"):
        if len(g) < 20:
            print(f"[SKIP] {product}: too few points {len(g)}")
            continue
        ...
        product_results[product] = {
            "fit_type": "quadratic_by_product",
            "formula": "fair_iv_product(m) = a*m^2 + b*m + c",
            "product": product,
            "K": int(g["K"].iloc[0]),
            "a": a,
            "b": b,
    ...
    if not fitted_frames:
        raise RuntimeError("No product fits produced.")

    return pd.concat(fitted_frames, ignore_index=True), product_results


def save_outputs(fitted: pd.DataFrame, product_results: dict, out_dir: Path) -> None:
    ...
    coeff_path = out_dir / "round3_iv_fit_coeffs_by_product.json"
    points_path = out_dir / "round3_iv_points_with_fit_by_product.csv"

    coeff_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    fitted.to_csv(points_path, index=False)

    # 1. product별 scatter + fitted curve
    for product, info in product_results.items():
        g = fitted[fitted["product"] == product].copy()
        coeffs = np.array(info["coeffs"], dtype=float)

        xs = np.linspace(g["m"].min(), g["m"].max(), 500)
        ys = np.polyval(coeffs, xs)

        plt.figure(figsize=(11, 7))
        plt.scatter(g["m"], g["raw_iv"], s=8, alpha=0.35, label=product)
        plt.plot(xs, ys, linewidth=2.5, label=f"{product} quadratic fit")
        plt.xlabel("m = log(K / S) / sqrt(TTE)")
        plt.ylabel("raw implied volatility")
        plt.title(f"Round 3 IV Fit by Product - {product}")
        plt.legend(fontsize=8)
        plt.tight_layout()
        plt.savefig(out_dir / f"round3_iv_fit_scatter_{product}.png", dpi=160)
        plt.close()
        ...
        plt.title(f"IV Fit Residuals - {product}")
        plt.tight_layout()
        plt.savefig(out_dir / f"round3_iv_fit_residuals_{product}.png", dpi=160)
        plt.close()

    # 2. 모든 product의 fitted curve 비교
    plt.figure(figsize=(11, 7))
    ...
    for product, info in product_results.items():
        ...
        xs = np.linspace(g["m"].min(), g["m"].max(), 300)
        ys = np.polyval(coeffs, xs)

        label = f"{product}: a={coeffs[0]:.4f}, b={coeffs[1]:.4f}, c={coeffs[2]:.4f}"
        plt.plot(xs, ys, linewidth=2.5, label=label)

    plt.xlabel("m = log(K / S) / sqrt(TTE)")
    plt.ylabel("fitted fair IV")
    plt.title("Round 3 Quadratic IV Curves by Product")
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(out_dir / "round3_iv_fit_curves_by_product.png", dpi=180)
    plt.close()

    # 3. residual by product
    products = [p for p in VOUCHERS if p in set(fitted["product"])]
    data = [fitted.loc[fitted["product"] == p, "residual"].to_numpy() for p in products]
    ...
    plt.ylabel("raw_iv - fit_iv")
    plt.title("Residual Distribution by Product")
    plt.tight_layout()
    ...
    print(f"[SAVED] {out_dir / 'round3_iv_fit_curves_by_product.png'}")


def main() -> None:
    repo_root = Path(__file__).resolve().parents[2]
    default_data_dir = repo_root / "data_capsule" / "round3"
    default_out_dir = Path(__file__).resolve().parent / "outputs" / "round3_iv_fit_by_product"
    ...
    fitted, product_results = fit_by_product(filtered)
    save_outputs(fitted, product_results, args.out_dir)
