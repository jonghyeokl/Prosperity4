...
PARAMS = {
    ...
    Product.KELP: {
        ...
        "adverse_volume": 15,
        "reversion_beta": 0.2953, # 기존 -0.229
        "disregard_edge": 1,
        "join_edge": 0,
        "default_edge": 1,
    },
...
class Trader:
    ...
    def kelp_fair_value(self, order_depth: OrderDepth, traderObject) -> float:
        ...
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
            ...
            if (traderObject.get("kelp_last_price", None) is not None) and (traderObject.get("kelp_last_last_price", None) is not None) and (traderObject.get("kelp_last_last_last_price", None) is not None):
                last_price = traderObject["kelp_last_price"]
                last_last_price = traderObject["kelp_last_last_price"]
                last_last_last_price = traderObject["kelp_last_last_last_price"]
                last_avg = (last_price + last_last_price + last_last_last_price) / 3
                fair = mmmid_price + self.params[Product.KELP]["reversion_beta"] * (last_avg - mmmid_price)
            else:
                fair = mmmid_price
            
            if(traderObject.get("kelp_last_last_price", None) is not None):
                traderObject["kelp_last_last_last_price"] = traderObject["kelp_last_last_price"]
            if traderObject.get("kelp_last_price", None) is not None:
                traderObject["kelp_last_last_price"] = traderObject["kelp_last_price"]
