...
PARAMS = {
    ...
    Product.SPREAD: {
        ...
        "spread_std_window": 230,
        "zscore_threshold": 3.55,
        "target_position": 20
    },
    Product.JAMS: {
        # "take_width": 2,
    ...
    Product.VOLCANIC_ROCK: {
        ...
        "take_best": True,
        "min_volume": 20,
        "zscore_range" : 123,  # z-score 범위: 200
        "zscore_threshold": 2.13,
        "target_position": 400,
        "default_std_val": 142,
    },
...
class Trader:
    ...
    def rolling_order(self, product,order_depths,positions, traderObject) -> float:
        ...
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
            ...
            if traderObject.get(product+"_target_position",None) and position != traderObject[product+"_target_position"]:
                if self.params[product].get("take_best",False):
                    return self.execute_target_orders_best_only(
                        product,
    ...
    def execute_target_orders_best_only(self,product,target_position, position,order_depth: OrderDepth):
        if target_position == position:
            return None
        orders = []
        position_diff = target_position - position
        ...
        if position_diff > 0:
            ...
            for ask_price, ask_volume in sorted_asks:
                ...
                if position_diff <= 0:
                    break
                if total_volume >= self.params[product]["min_volume"]:
                    break
    ...
    def getSignal(self,product, order_depths, market_trades,position, traderObject):
        trades = [t for t in market_trades if t.timestamp == traderObject["timeStamp"] - 100]
        
        teacher = self.params[product]["teacher"]
        target_position = traderObject.get(product+"_target_position", None)

        if any(t.buyer == teacher and t.seller != teacher for t in trades):
            target_position = self.params[product]["target_position"]
        elif any(t.seller == teacher and t.buyer != teacher for t in trades):
            target_position = -self.params[product]["target_position"]
        
        if target_position is None:
            return []
        traderObject[product+"_target_position"] = target_position
        return self.execute_target_orders_best_only(product, target_position, position, order_depths[product])
    ...
    def run(self, state: TradingState) -> (Dict[Symbol, List[Order]], int, str):
        ...
        current_round = state.timestamp / 1000000 + 5
        ...
        logger.flush(original_state, result, conversions, "")
