...
PARAMS = {
    ...
    Product.SPREAD: {
        "default_spread_mean": -400, # -14.76285
        "default_spread_std": 76.07966,
        "spread_std_window": 50, # 45
        "zscore_threshold": 3.25,
        "target_position": 20
    },
}
...
class Trader:
    ...
    def get_synthetic_basket_order_depth(self, order_depths: Dict[str, OrderDepth], spread_data) -> OrderDepth:
        ...
        spread_data["synthetic_order_depth"] = synthetic_order_depth
    ...
    def execute_spread_orders(self, target_position: int, synthetic_position: int,
        ...
        spread_data["target_position"] = target_position
        spread_data["synthetic_position"] = synthetic_position
        spread_data["target_quantity"] = target_quantity
        if target_position > synthetic_position:
            ...
            spread_data["ask_price"] = ask_price
            spread_data["available_volume"] = available_volume
            execute_volume = min(available_volume, target_quantity)
            spread_data["execute_volume"] = execute_volume
            ...
            spread_data["bid_price"] = bid_price
            spread_data["available_volume"] = available_volume
            execute_volume = min(available_volume, target_quantity)
            spread_data["execute_volume"] = execute_volume
    ...
    def spread_orders(self, order_depths: Dict[str, OrderDepth], synthetic_position: int,
        ...
        # spread_std: spread_data["spread_history"]의 각 원소에서 (float(np.mean(spread_data["spread_history"]))을 뺀 후 np.std 사용
    ...
    def run(self, state: TradingState) -> (Dict[Symbol, List[Order]], int, str):
        ...
        #     resin_take_orders, bo, so = self.take_orders(Product.RAINFOREST_RESIN,
        #                                                    state.order_depths[Product.RAINFOREST_RESIN],
        #                                                    self.params[Product.RAINFOREST_RESIN]["fair_value"],
        #                                                    self.params[Product.RAINFOREST_RESIN]["take_width"],
        #                                                    resin_position)
        #     resin_clear_orders, bo, so = self.clear_orders(Product.RAINFOREST_RESIN,
        #                                                      state.order_depths[Product.RAINFOREST_RESIN],
        #                                                      self.params[Product.RAINFOREST_RESIN]["fair_value"],
        #                                                      self.params[Product.RAINFOREST_RESIN]["clear_width"],
        #                                                      resin_position, bo, so)
        ...
        #     kelp_take_orders, bo, so = self.take_orders(Product.KELP,
        ...
        #     kelp_clear_orders, bo, so = self.clear_orders(Product.KELP,
        #                                                    state.order_depths[Product.KELP],
        #                                                    kelp_fair_value,
        ...
        #     squid_ink_orders = self.squid_ink_spread_order(state.order_depths[Product.SQUID_INK],squid_ink_position, traderObject)
        #     result[Product.SQUID_INK] = squid_ink_orders or []
        if Product.SPREAD not in traderObject:
            traderObject[Product.SPREAD] = {"spread_history": [], "prev_zscore": 0}
        synthetic_position = state.position.get(Product.JAMS, 0)
