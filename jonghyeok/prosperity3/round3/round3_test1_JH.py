...
class Trader:
    ...
    def volcanic_rock_voucher_orders(self, product, voucher_order_depth, voucher_position, traderData_entry, underlying_mid_price, current_round):
        ...
        if len(history) > params["std_window"]:
            history.pop(0)
        traderData_entry["mt_vol_history"] = history
        ...
        if base_iv is None:
            ...
            traderData_entry["deviation_history"] = deviation_history
    ...
    def run(self, state: TradingState) -> (Dict[Symbol, List[Order]], int, str):
        ...
        # ① underlying: VOLCANIC_ROCK 주문 장에서 mid price 계산
        ...
        # # ⑤ 개별 바우처마다 Black–Scholes에 따른 델타, 감마 계산을 통해 헷지 대상 포지션 산출
        # gamma_factor = 0.1  # 조정 가능
        # total_target_rock_position = 0
        # for voucher in voucher_list:
        #     voucher_pos = state.position.get(voucher, 0)
        #     params = self.params[voucher]
        #     tte = params["starting_time_to_expiry"] + 1 - current_round
        #     if tte <= 0:
        #         continue
        #     strike = params["strike"]
        #     voucher_mid = self.get_voucher_mid_price(state.order_depths[voucher], traderObject.setdefault(voucher, {}))
        #     if voucher_mid is None:
        #         continue
        #     voucher_iv = BlackScholes.implied_volatility(voucher_mid, rock_mid_price, strike, tte)
        #     delta_v = BlackScholes.delta(rock_mid_price, strike, tte, voucher_iv)
        #     gamma_v = BlackScholes.gamma(rock_mid_price, strike, tte, voucher_iv)
        #     target_for_voucher = - int((delta_v + gamma_factor * gamma_v * rock_mid_price) * voucher_pos)
        #     total_target_rock_position += target_for_voucher

        # # ⑥ VOLCANIC_ROCK 헷지 주문 생성 (현재 포지션과 목표 헷지 포지션의 차이 계산)
        # rock_position = state.position.get(Product.VOLCANIC_ROCK, 0)
        # rock_buy_orders = 0
        # rock_sell_orders = 0
        # hedge_qty = total_target_rock_position - (rock_position + rock_buy_orders - rock_sell_orders)
        # hedge_orders = []
        # if hedge_qty > 0:
        #     best_ask = min(rock_order_depth.sell_orders.keys())
        #     qty = min(hedge_qty, abs(rock_order_depth.sell_orders[best_ask]))
        #     qty = min(qty, PARAMS[Product.VOLCANIC_ROCK]["volume_limit"] - (rock_position + rock_buy_orders))
        #     if qty > 0:
        #         hedge_orders.append(Order(Product.VOLCANIC_ROCK, best_ask, qty))
        # elif hedge_qty < 0:
        #     best_bid = max(rock_order_depth.buy_orders.keys())
        #     qty = min(abs(hedge_qty), rock_order_depth.buy_orders[best_bid])
        #     qty = min(qty, PARAMS[Product.VOLCANIC_ROCK]["volume_limit"] + (rock_position - rock_sell_orders))
        #     if qty > 0:
        #         hedge_orders.append(Order(Product.VOLCANIC_ROCK, best_bid, -qty))

        # # ⑦ 만약 헷지 주문으로 인해 예상되는 VOLCANIC_ROCK 포지션이 포지션 한도에 도달하면 voucher_orders를 전송하지 않도록 함
        # if abs(rock_position + hedge_qty) >= PARAMS[Product.VOLCANIC_ROCK]["volume_limit"]:
        #     voucher_orders = []

        # ⑧ 결과 합산: voucher 주문과 헷지 주문 모두 result에 추가
        if voucher_orders:
            result.update({p: [] for p in voucher_list})
