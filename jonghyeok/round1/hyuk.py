    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            if product == "INTARIAN_PEPPER_ROOT":
                ...
                # get fair value
                avg_price = sum(traderObject["intarian_pepper_root_last_mid_price_history"]) / len(traderObject["intarian_pepper_root_last_mid_price_history"]) if len(traderObject["intarian_pepper_root_last_mid_price_history"]) > 0 else 0
                fair_value = avg_price + 0.1

                # calculate mid price
                mid_price = (max(order_depth.buy_orders.keys()) + min(order_depth.sell_orders.keys())) / 2 if order_depth.buy_orders and order_depth.sell_orders else 0
